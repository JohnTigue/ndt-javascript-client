...
NDTjs.prototype.findNdtServer = function () {
  ...
  if (mlabNsRequest.status === 200) {
    this.mlabServer = JSON.parse(mlabNsRequest.responseText);
    this.mlabServer.metro = this.mlabServer.site.slice(0, 3);
    this.logger('M-Lab NS lookup answer:' + this.mlabServer);
  } else {
    this.mlabServer = undefined;
    this.logger('M-Lab NS lookup failed.');
  }

  return this.mlabServer;
};
  ...
  return function (messageType, messageContent) {
    ...
    if (state === 'WAIT_FOR_TEST_MSG_OR_TEST_FINISH' &&
      ...
      var web100Record = messageContent.msg.split(': '),
        ...
        web100Result = web100Record[1].replace(/(\r\n|\n|\r)/gm, '');
